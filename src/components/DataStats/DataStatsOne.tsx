...
const LOW_QUANTITY_THRESHOLD = 10; // Low stock threshold
...
const DataStatsOne: React.FC = () => {
  ...
  const [stats, setStats] = useState({
    totalProducts: 0,
    lowStockCount: 0,
    lowStockPercentage: "0", // Percentage of low stock items as string
  });
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((response) => {
        ...
        const totalProducts = fetchedItems.length;
        const lowStockCount = fetchedItems.filter(
          (item) => item.Quantity < LOW_QUANTITY_THRESHOLD
        ).length;

        const lowStockPercentage =
          totalProducts > 0
            ? ((lowStockCount / totalProducts) * 100).toFixed(2)
            : "0";

        setStats({
          totalProducts,
          lowStockCount,
          lowStockPercentage, // Already a string
        });
  ...
  const dataStatsList = [
    ...
    },
    {
      ...
      color: "#FF0000",
      title: "Low Stock",
      value: `${stats.lowStockCount} (${stats.lowStockPercentage}%)`, // Ensures proper string formatting
    },
  ];
